...
function AuthClient(options) {
  ...
  Joi.assert(options, schema, {convert: false});
 ...
 * is an object with the property `vin`, Smartcar will only authorize the vehicle
...
AuthClient.prototype.getAuthUrl = function(options) {
  ...
  if (options.singleSelect !== undefined && options.singleSelect !== null) {
    if (typeof options.singleSelect === 'object') {
      ...
      /* eslint-disable camelcase */
      parameters.single_select = false;
      for (const param of availableParams) {
        if (param in options.singleSelect) {
          parameters['single_select_' + param] = options.singleSelect[param];
          parameters.single_select = true;
        }
      }
    } else {
      parameters.single_select = options.singleSelect === true;
      /* eslint-enable camelcase*/
    }
  }
